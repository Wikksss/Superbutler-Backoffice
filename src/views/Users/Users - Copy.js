...
class Users extends Component {
 ...
 this.setState({
  ...
  DeleteUserConfirmation(userId, name, roleLevel) {
    ...
    var adminUsers = this.state.UserList.filter(au => au.RoleLevel == Constants.Role.ENTERPRISE_ADMIN_ID)
 ...
 toggleLogoModal(user) {
    ...
    onZoomChange(zoom) {
      ...
      if(Utilities.stringIsEmpty(this.state.LogoImage)) return;
  ...
  <>
   <div onClick={()=> this.toggleLogoModal(user)} className="user-avatar-web">
              <Avatar  className="header-avatar" name={fullName} round={true} size="45" textSizeRatio={2} />
            </div>
            <div onClick={()=>this.toggleLogoModal(user)} className="user-avatar-res">
