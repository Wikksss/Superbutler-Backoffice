            ...
            <h3 className="card-title card-new-title d-flex  mb-0 pr-3 ">
              ...
              <div className='d-flex w-100  flex-md-row flex-column badge-override'>
                <div className='col-xs-12 mb-2 w-100'>
                  ...
                  {/* <div className="d-flex align-items-center mb-1"><span className="mr-2 font-14 mb-0">{this.SetPaymentMode(orderDetail.OrderPayments, orderDetail.OrderStatus, orderDetail.TotalAmount)}</span></div> */}
...
<div>
    ...
    <section className="inovice-detail">
        ...
        <div className="invoice-top-wraper">
            ...
            <div className="invoice-detail-inner-wraper bg-white" id="your">
                <div className="inovce-detail-block inovce-detail-block-new font-20 bold">{Utilities.SpecialCharacterDecode(orderDetail.Restaurant.Name)} </div>
  ...
  
  {
    ...
    <span className='bold'>  {this.SetPaymentMode(orderDetail.OrderPayments, orderDetail.OrderStatus, orderDetail.TotalAmount)} </span>
    </div>
                            ...
                            <div className="order-item-wraper">
                               ...
                               {AdditionalCharges != undefined && AdditionalCharges.length > 0 && AdditionalCharges.map((charge) => {
                                ...
                                <div className="order-total-payable-wrap">
                                    ...
                                    <span className="total-pay-rs">{orderDetail.Currency} {Utilities.FormatCurrency( this.isCardOrder(orderDetail.OrderPayments, orderDetail.TotalAmount) ? 0 :  Number(orderDetail.TotalAmount), this.state.countryConfigObj?.DecimalPlaces)}</span>
                                </div>
                                <div className="order-total-payable-wrap mt-2 font-weight-normal font-16" style={{paddingRight:"160px"}}>
                                    ...
                                    <span className="total-pay-rs font-14">{this.SetPaymentMode(orderDetail.OrderPayments, orderDetail.OrderStatus, orderDetail.TotalAmount)}</span>
                                </div>
                            </div>
                        </div>
                       {
                          ...
                          <div className="d-flex p-3 w-100">
                            <span className={`alert alert-${this.hasPaid(orderDetail.OrderPayments, orderDetail.OrderStatus, orderDetail.TotalAmount) ? "success" : "warning"} font-14 bold w-100 text-center mb-0`} style={{padding:"10px"}}>{this.hasPaid(orderDetail.OrderPayments, orderDetail.OrderStatus, orderDetail.TotalAmount) ? "ORDER HAS BEEN PAID" : "ORDER HAS NOT BEEN PAID"}</span>
                          </div>
                       }
            ...
            <div className="scroll-x" id="checkOrderActivity">
                <table>
                    ...
                    {this.state.EnterpriseActivity.map(activity => {
                        ...
                        <div className="font-16">{activity.Message}</div></td></tr>
                        )})
  }
