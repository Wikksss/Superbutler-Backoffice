...
setLoaderPercentageBeforeAPI = () => {
  ...
  this.timer = setInterval(() => {
    this.setState((prevState) => {
      ...
      if (prevState.percentage < 67) { // Target percentage
        var percentageNumber = prevState.percentage +  randomIncrement;
        return { percentage: percentageNumber > 67 ? 67 : percentageNumber};
      } else {
...
setLoaderPercentageAfterAPI = () => {
  ...
  createdemo = async(event, values) => {
    ...
    this.setState({validUser: valid, haveSavingData: true, demoData: demo, isInvalidPin: false, pin: ""});
    if(!valid) return;

    this.createDemoAPI(demo);
    

}

createDemoAPI = async (demo) => {
  ...
  this.setState({creatingHotel: true, haveSavingData: false}, () => this.setLoaderPercentageBeforeAPI());
  ...
  if (!response.HasError && response !== undefined) {
    ...
    if (response.Dictionary.IsCreated) {
      this.setState({subDomainName: response.Dictionary.SubDomainName, hotelName:  Utilities.SpecialCharacterDecode(demo.Name)})
...
ValidatePin = async (pin, load) => {
  ...
  let isValid = await Enterprise.ValidatePin(pin);
  localStorage.setItem("PinCode", pin);
  this.setState({validUser: isValid, isInvalidPin: !isValid});
  
  if(isValid && this.state.haveSavingData){
    this.createDemoAPI(this.state.demoData);
  }
  if(load) this.setState({isInvalidPin: false});
...
IsDemoValidation = async () => {
  ...
  if (response) {
    ...
    {
      var validate = await this.ValidatePin(pin, true);
      if(!validate)
        {
          this.setState({validUser: false, showLoader: false})
          return;
        } else 
        {
          this.GetEnterprises();
...
onChangePIN = (value) => {
  ...
  this.setState({pin: value, isInvalidPin: false});
}
...
{ !this.state.validUser &&   <div>
              ...
              <AvForm onValidSubmit={() => this.ValidatePin(this.state.pin, false)}>
...
<></>
      {!this.state.creatingHotel && !this.state.showDetail && this.state.validUser && 
...
{ this.state.showDetail && this.state.validUser && 
...
<Button className='create-another-htl-btn' color="primary" onClick={() => this.CreateHotel()}>
 ...
 <span className="comment-text">Create another</span>
          ...
          { this.state.creatingHotel &&  this.state.validUser && 
          <div>
           <div className='d-flex align-items-center justify-content-center demo-page-logo-wrap mt-4'>
           <img alt='Superbutler'style={{height:"auto"}} width={200} src={demoHotelFooterImage}></img>
       </div>
