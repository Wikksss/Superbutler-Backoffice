...
<div className="loader-menu-inner">
  ...
  connectQZ = async () => {
    try {
      if (!window.qz) {
        alert("QZ Tray is not loaded. Please install QZ Tray.");
        return;
      }
...
// }
  ...
  printCustomerCopy = async (customerCopy) => {
    ...
    // const config = window.qz.configs.create("EPSON TM-T70II Receipt (1)");
    // // Extract HTML content from a div
    // const htmlContent = document.getElementsByClassName("luggage-print-rec-wrap").innerHTML;
 
    // // Wrap the content in proper HTML format
    // const formattedHTML = `
    //   <html>
    //     <head>
    //       <style>
    ...
    // try {
    //   await window.qz.print(config, [{ type: "html", format: "plain", data: formattedHTML }]);
    //   alert("HTML Print successful!");
    // } catch (err) {
    //   console.error("HTML Print failed:", err);
    // }
...
SetPaymentMode = (paymentModes, status, payableAmount) => {
    ...
    <div className='luggage-print-rec-wrap'>
        ...
        { this.state.customerCopy ? 
            <div className='luggage-hdr-1 text-center'>
              <img className='common-title' style={{marginBottom:"15px"}} height={60} src={Utilities.generatePhotoLargeURL(order.Restaurant.PhotoName, true, false)} />
  ...
  <a className="btn btn-primary hidden-print text-white w-100" style={{marginTop:'0px'}} target="_blank" onClick={() => this.printCustomerCopy(false)}>
					...
					{order.Currency}
                          ...
                          <>
                            ...
                            <tr>
                             ...
                             <tr>
                                <td className="right amount-bold">{Labels.Items_Cost_After_Discount}:</td>
                                <td className="amount-bold" colSpan="2">{Utilities.FormatCurrency(Number(order.TotalAmountExTax) - Number(order.DiscountedAmount), this.state.countryConfigObj?.DecimalPlaces)}</td>
                            </tr>
                            </>
                            :
                            <tr>
...
{/* 
                          ...
                          { Utilities.hasPaid(order.OrderPayments, order.OrderStatus, order.TotalAmount) &&
                                 <>
                                  <tr>
                                      <td colSpan="2"><p className="border-dash small">-------</p> </td>
                                  </tr>
                                  <tr>
                                      <td className="right amount-bold">{Labels.Order_Total}:</td>
                                  ...
                                  <tr>
                                      <td className="right amount-bold">{Labels.Paid_By} <span>({Utilities.SetPaymentMode(order.OrderPayments, order.OrderStatus, order.TotalAmount)})</span></td>
                                      <td className="amount-bold" colSpan="2">- {Utilities.FormatCurrency(Number(order.TotalAmount), this.state.countryConfigObj?.DecimalPlaces)}</td>
                                  </tr>
                            ...
                            <tr>
                                ...
                                <td className="amount-bold" colSpan="2">{Utilities.FormatCurrency(Utilities.isCardOrder(order.OrderPayments, order.TotalAmount) ? 0 :  Number(order.TotalAmount), this.state.countryConfigObj?.DecimalPlaces)}</td>
								</tr>
